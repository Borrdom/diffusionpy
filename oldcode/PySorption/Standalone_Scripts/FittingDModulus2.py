...
par=Parameters()
nArm=4
tauJ0=np.linspace(-2,2,nArm)
[par.add('tauJ'+str(i), value=val, min=-5, max=5,vary=1) for i,val in enumerate(tauJ0)]
[par.add('EJ'+str(i), value=1E9, min=0, max=1E11) for i in range(nArm)]
par.add('Eeq',value=0,min=0, max=1E9,vary=0)
par.add('etaeq',value=0,min=0, max=1E9,vary=0)
...
def FitData(par,logomega,storage,loss):
    residual=lambda par,logomega,storage,loss: divide(np.asarray(GenMaxwell(par,logomega,storage,loss))+np.asarray([storage,loss]))-divide(np.asarray([storage,loss]))
    print(residual(par,logomega,storage,loss))
...
for i,val in enumerate(Data):
    ...
    # storagesim,losssim=storagesim+storage,losssim+loss
    #Enter Your Model
    om=np.linspace(-5,5,300)
    para=Parameters()
    nArm=1
    para.add('tauJ0', value=np.log10(10))
    para.add('EJ0', value=2E9)
    ...
    ax[0,i].plot(logomega,storage,'rx')
    ax[0,i].plot(logomega,loss,'bx')
    ax[0,i].plot(om,aopt,'r-')
    ax[0,i].plot(om,bopt,'b-')
