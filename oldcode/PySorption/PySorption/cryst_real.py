...
def PCSAFT(temp,polstr,apistr,solstr,Dw,dl):
    ...
    pure, kij,ksw = get_par(excelstr,T=temp)
    ...
    if not os.path.isfile(pathname_1):
        ...
        result_napvp_sle = [napvp.SLE(psys=1.013E5,T=temp,wwASD=val) for i,val in enumerate(ww_SAFT)] #DB Ich dachte, dass sich im ternären das chem. Pot ändern muss da sich die Löslichkeit mit der Wasserkonzentration ändern!
        ...
        napvp.WriteToExcel([result_napvp_sle,result_napvp_sle],["SLE","SLE"],Datatype="SLE")
    else:
        None
    ...
    w_lw_SAFT_eq = df_eq.loc[:-1, "wi"+str(napvp.idx)].values
    w_la_SAFT_eq = df_eq.loc[:-1, "wi"+str(napvp.idxa)].values

    #dl_eq=w_la_SAFT_eq/(1-w_lw_SAFT_eq)
    #dl_eq_int=InterpolatedUnivariateSpline(w_lw_SAFT_eq,dl_eq,k=2)
    ...
    mu_la_SAFT_0=mu_la_SAFT[w_la_SAFT==1]
    #a_la_SAFT=np.exp(mu_la_SAFT-mu_la_SAFT_0) 
    #a_la_SAFT_eq=np.exp(mu_s_SAFT_eq-mu_la_SAFT_0) 
    #mu_la_SAFT =df_scan.loc[:, "mui"+str(napvp.idxa)].values
    #_la_SAFT = df_scan.loc[:, "wi"+str(napvp.idxa)].values
    ...
    return dmu_int,excelstr,mu_la_SAFT_0,mu_s_SAFT_eq
...
def cryst(rho, M, temp, sigma, kt, g, D, Dw=0):
    ...
    # scale_r =1E5
    # scale_N =1E-4
    scale_r=(m_ges_0/0.1)**3 #DB: Das was ich bei Whatsapp meinte. Du musst das variable mit der Masse Skalieren
    scale_N=(m_ges_0/0.1)**3 #DB: Denk dir das so. Der SweetSpot war bei mges=0.1 hier waren rscale und Nscale 0.
    ...
    dmu_int,excelstr,mu_la_SAFT_0,mu_s_SAFT_eq=PCSAFT(temp,polstr,apistr,solstr,Dw,dl)

    dmu_sla_0 = dmu_int(cs.vertcat(w_la_0,ww0)) if len(excelstr)>2 else dmu_int(w_la_0)



    r0 = ((2*sigma*v_a_0)/(R*temp*dmu_sla_0))/scale_r #critical diamter
    #r0 = 1.40195E-8 #Aspect ratio from a needle crystal should be around 1:100-1000

    # Defining the two main casadi expressions 'ms' and 'w and their respective functions:
    
    #rneedle=((4*sigma*v_a_0)/(R*temp*dmu_sla_0))/scale_r
    rneedle=1/100*r_strich/2
    print(rneedle)
    #rneedle=r0
    m_sa_strich =rho*pi*((rneedle)**2)*(r_strich)*N_strich*NA #rho*(4/3)*pi*(r_strich**3)*N_strich*NA #Unit: kg*molecule/mol
    ...
    if Dw>0:
        ...
        l=m_ges_0/A/1200 #in Mikrometer-Range - realistisch? DB: Ja 15-40 micrometerfilme haben wir
        ...
        if len(excelstr)>2:
            ...
            cw_infty_vec=InterpolatedUnivariateSpline(dlvec_exp[::-1],cw_infty_exp[::-1],k=2)(dlvec_vec)
            ...
            # Kinetik schwingt nach unten Wenn Messdaten Kugel oder nadel kein Unterschied

            #dl_la = (m_la_0-m_sa)/(m_la_0-m_sa+mp)
            dl_la = (dl-alpha)/(1-alpha)
        ...
        ww=cw/(1+cw)
        ww_la=beta*cw_la/(1+beta*cw_la)
        ww_fun = cs.Function("ww_fun",[N,r,T],[ww])
        dl_fun = cs.Function("ww_fun",[N,r,T],[dl_la])
        mw =cw*m_ges_0 #
        #mw=(ww*m_ges_0)/(1-ww)
        #mw=0
    else:
    ...
    dmu_sla =dmu_int(cs.vertcat(dl_la,ww_la)) if len(excelstr)>2 else dmu_int(w_la) #Variable Übersättigung "springt" rauf und runter
    #mu_sla=dmu_sla+mu_s_SAFT_eq
    a_la =np.exp(dmu_sla + mu_s_SAFT_eq-mu_la_SAFT_0)
    a_la_SAFT_eq=np.exp(mu_s_SAFT_eq-mu_la_SAFT_0) 
    print(a_la_SAFT_eq)
    ...
    #V_l=m_l/rho
    #_sa_eq=
    #m_la_eq=m_la_0-m_sa_eq
    #n_la_eq=m_la_eq/M
    c_la = dl_la*(1-ww_la)#m_la/(m_ges_0-m_sa+mw)
    # DB:An sich kürzt sich das Vs in der ODE sowieso aus. Deswegen brauchen wir das gar nicht mehr
    ...
    #a_sphere = (36*pi*v_a_0**2)**(1/3)
    nstar = (8/27)*(a*sigma/(dmu_sla*R*temp))**3 #Von Moritz. ist wie bei Kashchiev
    ...
    #Wstar=1/2*nstar*dmu_sla aus Kashchiev sieht gut aus
    #Wstar=4/27*(a*sigma)**(3)*dmu_sla**(2) Clouet
    ...
    fstar_dif_sym = c_fstar_dif*D*n_la*(nstar_sym**(1/3))
    ...
    fstar_inter_sym = c_fstar_inter*D*n_la*(nstar_sym**(2/3))
    ...
    z = (-dWstardnstar_2/(2*pi*R*temp))**(1/2) #Über automatische Ableitung
    #z = (dmu_sla/(4*pi*R*temp*nstar))**(1/2) # 2005 Jansens. Beide liefern dieselben Ergebnisse.
    z = (dmu_sla/(4*pi*nstar))**(1/2) # 2005 Jansens. Delta mu ist ja normiert
    #z = (dmu_sla/6)**(1/2)
    #z=(Wstar/(3*pi*R*T*nstar**2))**(1/2) #Kashchiev 2003 and Vehkamäki 2007
    #z=(3*dmu_sla**(2)/(4*(a*sigma)**(3/2)*(pi)**(1/2)*(R*T)**(7/6))) #Clouet
    #z=(dmu_sla**2/(64*pi**2*R*T))**(1/2)*sigma**(3/2)*v_a_0
    

     #passt der ausdruck aus irgendeiner Quelle damit zusammen?
    ...
    #P1=sigma**(0.5)*c_a_0*D*n_la*R*temp #almost activity in SLE is missing
    P1=(48*pi**2*v_a_0)**(1/3)*D*n_la*(2/3*a*sigma/(R*temp))*(1/(4*pi*(2/3*a*sigma/(R*temp))**3))**(1/2)*c_a_0
    #P1=(12*pi**(0.5)*v_a_0**(-2))**(1/3)*D*n_la*(2/3*a*sigma/(R*temp))**(-0.5)
    #P1=3**(1/2)*2**(-1/6)*c_a_0*D*(R*temp)**(1/2)*sigma**(-0.5)*a_la_SAFT_eq # is good
    #P10=3**(1/2)*2**(-1/6)*c_a_0*D0*(R*temp)**(1/2)*sigma**(-0.5)*a_la_SAFT_eq
    P1=c_a_0*D*(R*temp)**(1/2)*sigma**(-0.5)#*a_la_SAFT_eq # is good
    P10=c_a_0*D0*(R*temp)**(1/2)*sigma**(-0.5)#*a_la_SAFT_eq
    #P1=3**(1/2)*2**(-1/6)*c_a_0*D*(R*temp)**(1/2)*sigma**(-0.5)*n_la # is good
    P2=4/27*(a*sigma)**3
    print(P10)
    print(P2)

    #dNdt = cs.fmax(P1*dmu_sla*exp(dmu_sla)*exp(-P2/dmu_sla**2),0)
    c_GGW=0.03*(1-0.24958)
    dmu_sla=cs.log(c_la/c_GGW) #ideal
    deltaG=(16*pi*v_a_0**2)/(3*(R*temp*dmu_sla)**2)
    z=(R*temp)**(1.5)/(8*pi*v_a_0)*dmu_sla**2/(sigma)**1.5
    rstar=2*v_a_0*sigma/(dmu_sla*R*temp)
    f=4*pi*rstar*D*n_la
    dNdt = cs.fmax(1/scale_N*z*f*c_a_0*exp(-deltaG/(R*temp)),0)


    #dNdt = cs.fmax(P1*n_la*dmu_sla*exp(-P2/dmu_sla**2),0)
    g2=cs.conditional(cs.exp(dmu_sla)>2,[2],g)
    g2=g
    drdt = cs.fmax(1/scale_r*kt*(dmu_sla)**g2,0)
    #drdt = cs.fmax(1/scale_r*kt*cs.tanh(1/dmu_sla*g)*dmu_sla**2,0)
    #drdt = cs.fmax(1/scale_r*kt*cs.exp(-1/dmu_sla*g)*dmu_sla**(5/6),0)
    #drdt = cs.fmax(1/scale_r*kt*cs.exp(-1/dmu_sla*g),0)
    ...
    N0=1/NA/scale_N*0#ms0/M #zero is ok
    #r0 = ((2*sigma*v_a_0)/(R*temp*dmu_sla_0))/scale_r
    print(r0)
    N0 = cs.DM(N0)
    r0 = cs.DM(r0)*0 #zero makes no difference

    # DGL Solving Properties
    ts = np.linspace(0,tend**(1/2),tsteps+1)**2
    # x0 = [N0,r0]
    ...
    dl_vec =dl_fun(N_vec,r_vec,ts).full().T[0]
    ...
    cryst_output_dic = {}
    pa_cryst_output_keys = ['t_vec','N_vec','r_vec','alpha_vec','S_vec','dmu_vec','dNdt_vec','drdt_vec','nstar_vec','z_vec','fstar_dif_vec','fstar_inter_vec']
    aw_cryst_output_keys = ['t_vec','N_vec','r_vec','alpha_vec','S_vec','dmu_vec','dNdt_vec','drdt_vec','nstar_vec','z_vec','fstar_dif_vec','fstar_inter_vec','ww_vec','dl_vec']
...
def crystfit(texp,sigma,kt,g,D):
    ...
    M=0.23
    temp=298.15
    out=cryst(rho, M, temp, sigma, kt, g, D, Dw=0.6E-13)
    tsim,alphasim=out['t_vec'],out['ww_vec']
    alpha_fun=InterpolatedUnivariateSpline(tsim,alphasim)
    return alpha_fun(texp)

if __name__ == "__main__":

    #cryst_output_dic=cryst(1200, 0.23, 298.15, 5e-1, 8.2e-09, 1, 1E-16,2E-13) # Wenig S-form
    ...
    #tstr="tges[min]0"
    #ystr="wges[-]ave"
    #t_vec_exp,y_vec_exp=data.getrows(tstr,ystr)
    #popt,pcov=curve_fit(crystfit,t_vec_exp*60,y_vec_exp,p0=[0.05,5*10**-11,2,5*10**-19],bounds=([0.01,1*10**-14,0.01,1*10**-21],[0.1,1*10**-10,4,1*10**-15])) 
    #[1.33569184e-02 1.85767412e-11 2.07548213e+00 1.23772729e-16] # Startwerte
    #[1.67117123e-02 8.57296007e-12 1.58688190e+00 8.34221838e-17] # Bigger crystals
    #print(popt)
    #cryst_output_dic=cryst(1200, 0.23, 298.15, 5e-3, 8.2e-08, 4, 1E-15,0.6E-13)
    #cryst_output_dic=cryst(1250, 0.23, 298.15, popt[0], popt[1], popt[2], popt[3],0.6E-13)
    
    #r0 auf Molekülgröße -> 0.2 µm Lange Nadeln
    
    #sigmafit=3.99336372e-03 
    #ktfit=6.05818823e-13
    #gfit=2.10869485e+00 
    #Dfit=5.82104492e-16
    #Dwfit=0.6E-13
    #BAS
    #sigmafit=3.76968181e-03  
    #ktfit=3.4189803491482999e-11
    #gfit=0.6
    #Dfit=9.99944652e-16
    #Dwfit=0.6E-13

    ##NET-GP neu##
    #sigmafit=8.27656533e-03
    #ktfit= 5.43349150e-08
    #gfit= 3.43652552e+00 
    #Dfit=4.65314715e-16
    #Dwfit=0.6E-13 # Mittel 55 97 90
    #P1=1.3286470342563999e-09
    #P2=3.489733181807709e-13
    #Dwfit=0.3E-13 #Dick 76
     #dick 103 oder 96 
    ##NET-GP alt##
    #sigmafit=3.76968181e-03
    #sigmafit=5.76968181e-02 
    #ktfit=7.36482249e-08 ohne molare volumen
    #ktfit=1.4189803491482999e-11
    #ktfit=1.4189803491482999e-11
    #gfit=1.92847562e+00
    #gfit=2
    ...
    ## new base for needles
    sigmafit=2.97730286e-02
    ktfit=2.0778700e-12
    #ktfit=6e-10 equal fit BAS
    gfit=3.92 #failed Jacobian
    #gfit=6 equal fit BAS
    Dfit=6.59010330e-19
    Dwfit=0.6E-13
    ...
    cryst_output_dic=cryst(1250, 0.23, 298.15, sigmafit,ktfit,gfit,Dfit,Dwfit)
    ...
    #print(cryst_output_dic["z1_vec"]-cryst_output_dic["z2_vec"])
    fig,ax=plt.subplots()
    ...
    ww=cryst_output_dic["ww_vec"]
    Xw=ww/(1-ww)
    alpha=cryst_output_dic["alpha_vec"]
    Xwl=Xw/(1-alpha)
    wwl=Xwl/(Xwl+1)
    ax.plot(cryst_output_dic["t_vec"]/60,ww)
    ax.set_ylabel("ww")
    ax1.plot(cryst_output_dic["t_vec"]/60,cryst_output_dic["alpha_vec"])
    ax1.set_ylabel="alpha_vec"
    ax2.plot(cryst_output_dic["t_vec"]/60,cryst_output_dic["r_vec"])
    ax2.set_ylabel("r_vec")
    ax3.plot(cryst_output_dic["t_vec"]/60,cryst_output_dic["dl_vec"])
    ax3.set_ylabel("dl_vec")
    ax4.plot(cryst_output_dic["t_vec"]/60,wwl)
    ax4.set_ylabel("wwL_vec")
    ax5.plot(cryst_output_dic["t_vec"]/60,cryst_output_dic["N_vec"])
    ax5.set_ylabel("N_vec")
    ax6.plot(cryst_output_dic["t_vec"]/60,cryst_output_dic["drdt_vec"])
    ax6.set_ylabel("drdt_vec")
    plt.show()

    import pandas as pd
    pd.DataFrame((cryst_output_dic["t_vec"]/60,cryst_output_dic["dmu_vec"],1E6*cryst_output_dic["r_vec"],cryst_output_dic["ww_vec"])).T.to_excel("fil.xlsx")
    #pd.DataFrame(cryst_output_dic["t_vec"]/60,cryst_output_dic["r_vec_nm"]).to_excel("fil.xlsx")
