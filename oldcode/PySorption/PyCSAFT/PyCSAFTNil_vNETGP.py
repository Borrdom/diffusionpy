...
class Mixture:
    ...
    def PCSAFT(self,psys=0.03166*1E5*0.5,T=298.15,na=""):
        ...
        # I1,I2=cs.SX.zeros(ncomp,ncomp),cs.SX.zeros(ncomp,ncomp)
        # for i in range(ncomp):
        #     for j in range(ncomp):
        #         a=a0[0,:]+(mi[i]-1)/mi[i]*a0[1,:]+(mi[i]-1)/mi[i]*(mi[j]-2)/mi[j]*a0[2,:] #The integral for Segment based is now for every segment and inside the sum
        #         b=b0[0,:]+(mi[i]-1)/mi[i]*b0[1,:]+(mi[i]-1)/mi[i]*(mi[j]-2)/mi[j]*b0[2,:]
        #         I1[i,j]=cs.polyval(a[::-1],z3)*mi[i]*xi[i]/mibar*mi[j]*xi[j]/mibar
        #         I2[i,j]=cs.polyval(b[::-1],z3)*mi[i]*xi[i]/mibar*mi[j]*xi[j]/mibar
        # I1=cs.sum1(cs.sum2(I1))
        # I2=cs.sum1(cs.sum2(I2))
        #
        #I1,I2=cs.SX.zeros(ncomp,ncomp),cs.SX.zeros(ncomp,ncomp)
        # for i in range(ncomp):
        #     for j in range(ncomp):
        #         a=a0[0,:]+(mi[i]-1)/mi[i]*a0[1,:]+(mi[i]-1)/mi[i]*(mi[j]-2)/mi[j]*a0[2,:] #The integral for Segment based is now for every segment and inside the sum
        #         b=b0[0,:]+(mi[i]-1)/mi[i]*b0[1,:]+(mi[i]-1)/mi[i]*(mi[j]-2)/mi[j]*b0[2,:]
        #         I1[i,j]=cs.polyval(a[::-1],z3)*xi[i]*xi[j]
        #         I2[i,j]=cs.polyval(b[::-1],z3)*xi[i]*xi[j]
        # I1=cs.sum1(cs.sum2(I1))
        # I2=cs.sum1(cs.sum2(I2))
        ...
        #fdisp=-2*cs.pi*rho*I1*m2es3-cs.pi*rho*C1*I2*m2e2s3#*mibar
        ...
        self.Jnetrho=(self.rho0Blend*1000000-1/self.rhoblend*1000000)
    ...
    def rhoPolysys(self,psys=0.03166*1E5*0.5,T=298.15):
        ...
        #return rhoF*p_fun
        #return vF*p_fun
        return vF+cs.sum1(wFeed*v0Poly0vec*kswvec*(psys/self.pH2OLV)**nsw)
...
if __name__=="__main__":
    T=298.15
    pure, kij=get_pcpar.get_par(["pvp","water","indomethacin"],T=T)
    #
    ...
    kij["waternaproxen"]=0.1
    kij["naproxenwater"]=0.1
    ...
    #kij["pvpva64indomethacin"]=-0.0621 #aus Verhältnis der monomeranteile ausgerechnet
    #kij["indomethacinpvpva64"]=-0.0621
    ...
    #Film.Isohumide(T=298.15,main=True,NET=True,RHsup=RHsup)
    #Film.MultIsothermOrigin(Lit=1)
    #plt.show()
    #Film.LLE(ws=0)
    #plt.show()
    ...
    Film.Isotherm(0.5,niso=30,NET=1)
