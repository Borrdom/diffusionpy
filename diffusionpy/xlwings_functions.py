...
@xw.func
def Diffusion_MS_xw(t,L,Dvec,w0,w8,Mi,volatile,full_output=None,Gammai=None,swelling=None,taui=None):   
...
@xw.func
def BHX_xw(t,kf,kr,ws0,ws8,mfinfty,mrinfty):
    Xs0=ws0/(1-ws0)
    Xs8=ws8/(1-ws8)
...
@xw.func
def DasDennis_xw(p:int,dim:int):
...
#@xw.func
def get_par_xw(subst_input,path):
    wb=xw.active_workbook()
    sheet1=wb.add(name="Parameters_PC_SAFT")
    xw.Range("Parameters_PC_SAFT!A1").value="PC-SAFT parameters"
    xw.Range("Parameters_PC_SAFT!A2").value="Number"

    pure,kij=get_par(subst_input,path)
    cell1=xw.Range("Parameters_PC_SAFT!B2:Z2")
    cell2=xw.Range("Parameters_PC_SAFT!A3:Z"+str(2+len(pure)))
    cell3=xw.Range("Parameters_PC_SAFT!A14:F14")
    cell4=xw.Range("Parameters_PC_SAFT!A15:F"+str(14+(len(pure)-1)*len(pure)//2))
...
def create_header(ctrl):
    cell1=xw.Range("Parameters_PC_SAFT!B3:B13")
    comps=cell1.trim().value.flatten()
    x=np.ones_like(comps).astype(float)
    x=x/x.shape[0]
    xw.Range("A1:H1").value=np.asarray(["Fraction type"]+list(comps)+["Pressure [bar]","Temp. [K]","State (l/v) [-]","Density [kg/m³]"])[:,None].T
    xw.Range("A2:G2").value=np.asarray(["x"]+list(x)+[1,298.15,"l"])[:,None].T
    xw.Range("H2").formula="PC_SAFT_NpT(Parameters_PC_SAFT!$A$2:$Y$13;Parameters_PC_SAFT!$A$14:$F$59;A$1:Z$1; A2:G2)"

try:
    _excelgui = xw.ExcelGUI(ribbon=r'''
