...
def drhodt(t,rhov,tint,Gammai,taui,volatiles,nonvolatiles,nc,ri,D,allflux,swelling,nz,rho,rhoiinit,w0,w8):
    ...
    for i in range(nc):
        ...
        drhoidt[i,:]=djib
    if taui[0]!=0.: drhoidt[volatiles,-1]=1./taui*np.exp(-t/taui)*(w8[volatiles]-w0[volatiles])*rho
...
def Diffusion_MS(t,L,Dvec,w0,w8,Mi,volatile,full_output=False,Gammai=None,swelling=False,taui=None):
    ...
    rhoiinit[nonvolatiles,-1]=rho*w8[nonvolatiles]
    ...
    def wrapdrhodt(t,rhov,tint,GammaiT,taui,volatiles,nonvolatiles,nc,ri,D,allflux,swelling,nz,rho,rhoiinit,w0,w8):
        rhov=np.ascontiguousarray(np.reshape(rhov,(nTH,nz+1)))
        drhovdt=drhodt(t,rhov,tint,np.ascontiguousarray(GammaiT),taui,volatiles,nonvolatiles,nc,ri,D,allflux,swelling,nz,rho,rhoiinit,w0,w8)
        return np.reshape(drhovdt,np.multiply(*drhovdt.shape))
    
    print("------------- Start diffusion modeling ----------------")
    start=time.time_ns()
    sol=solve_ivp(wrapdrhodt,(t[0],t[-1]),xinit,args=(t,GammaiT,taui,volatiles,nonvolatiles,nc,ri,D,allflux,swelling,nz,rho,rhoiinit,w0,w8),method="Radau",t_eval=t)
    end=time.time_ns()
    print("------------- Diffusion modeling took "+str((end-start)/1E9)+" seconds ----------------")
    if not sol["success"]: print("------------- Modeling failed the initial conditions are returned instead ----------------"); return w0*np.ones((nc,nt)).T 
    x_sol=sol["y"] 
    nt=len(t)
    wt=np.zeros((nc,nt))
...
def Diffusion_MS_iter(t,L,Dvec,w0,w8,Mi,volatile,full_output=False,Gammai=None,swelling=False,taui=None):
    Gammaiave=np.stack([DlnaDlnx(T,vpures,wi8*0.5+wi0*0.5,mi,sigi,ui,epsAiBi,kapi,N,Mw,kij)]*nt).T
    wt=Diffusion_MS(t,L,Dvec,w0,w8,Mi,volatile,Gammai=Gammaiave,swelling=swelling,taui=taui)
    while np.linalg.norm(wt-wt1)<1E-4:
        Gammai=np.asarray([DlnaDlnx(T,vpures,np.ascontiguousarray(wt[i,:]),mi,sigi,ui,epsAiBi,kapi,N,Mw,kij) for i in range(nt)]).T
        #if i<5: Gammai=(Gammai*19/20+np.stack([np.eye(nc)]*nt).T*1/20) 
        wt1=Diffusion_MS(t,L,Dvec,wi0,wi8,Mi,volatile,Gammai=Gammai)
...
if __name__=="__main__":
    ...
    Gammaiave=np.stack([DlnaDlnx(T,vpures,wi8*0.5+wi0*0.5,mi,sigi,ui,epsAiBi,kapi,N,Mw,kij)]*nt).T
    wt=Diffusion_MS(t,L,Dvec,wi0,wi8,Mi,volatile,Gammai=Gammaiave)
    for i in range(5):
        plt.plot(t,wt[:,0])
        plt.plot(t,wt[:,1])
        plt.plot(t,wt[:,2])
    ...
    Dvec=np.asarray([1.7E-11])
    nc=2
    L=2E-5
    wi0=np.asarray([0.5,0.5])
    wi8=np.asarray([0.1,0.9])
    Mi=np.asarray([32.042,90000.])
    volatile=np.asarray([True,False])
    wt=Diffusion_MS(t,L,Dvec,wi0,wi8,Mi,volatile,swelling=True,taui=np.asarray([50.]))
    # plt.plot(t,wt[:,0])
    # plt.plot(t,wt[:,1])
    # plt.plot(t,wt[:,2])
    plt.show()