...
results=[ares(T,val,np.ascontiguousarray(xi[:,i]),mi,sigi,ui,epsAiBi,kapi,N) for i,val in enumerate(etamix)]
end=time.time_ns()
print((end-start)/1E9)
start=time.time_ns()
lnGammai=np.asarray([THFaktor(T,vpures,np.ascontiguousarray(xi[:,i]),mi,sigi,ui,epsAiBi,kapi,N) for i,val in enumerate(xi[0,:])])
end=time.time_ns()
print((end-start)/1E9)
start=time.time_ns()
