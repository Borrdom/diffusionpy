...
from .PyCSAFT_nue import SAFTSAC,vpure,etaiter,DlnaDlnx,ares,lnphi_TP
...
kijAB=np.asarray([[0.]])
start=time.time_ns()
lngammai=np.asarray([SAFTSAC(T,vpures,np.ascontiguousarray(xi[:,i]),mi,sigi,ui,epsAiBi,kapi,N,kij=np.asarray([[0.01]])).flatten() for i,val in enumerate(xi[0,:])])
end=time.time_ns()
print((end-start)/1E9)
start=time.time_ns()
etamix=np.asarray([etaiter(p,T,np.ascontiguousarray(xi[:,i]),mi,sigi,ui,epsAiBi,kapi,N) for i,val in enumerate(xi[0,:])])
results=[ares(T,val,np.ascontiguousarray(xi[:,i]),mi,sigi,ui,epsAiBi,kapi,N,kij,kijAB) for i,val in enumerate(etamix)]
end=time.time_ns()
print((end-start)/1E9)
start=time.time_ns()
lnGammai=np.asarray([DlnaDlnx(T,vpures,np.ascontiguousarray(xi[:,i]),mi,sigi,ui,epsAiBi,kapi,N,idx=-1) for i,val in enumerate(xi[0,:])])
end=time.time_ns()
print((end-start)/1E9)
start=time.time_ns()
