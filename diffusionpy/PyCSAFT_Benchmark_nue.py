...
kijAB=np.asarray([[0.]])
start=time.time_ns()
lngammai=np.asarray([lngi(T,vpures,xi[:,i],mi,sigi,ui,epsAiBi,kapi,N,kij=np.asarray([[0.01]])).flatten() for i,val in enumerate(xi[0,:])])
end=time.time_ns()
print((end-start)/1E9)
start=time.time_ns()
etamix=np.asarray([eta_iter(p,T,xi[:,i],mi,sigi,ui,epsAiBi,kapi,N) for i,val in enumerate(xi[0,:])])
results=[ares(T,val,xi[:,i],mi,sigi,ui,epsAiBi,kapi,N,kij,kijAB) for i,val in enumerate(etamix)]
end=time.time_ns()
print((end-start)/1E9)
start=time.time_ns()
lnGammai=np.asarray([dlnai_dlnxi(T,vpures,xi[:,i],mi,sigi,ui,epsAiBi,kapi,N,idx=-1) for i,val in enumerate(xi[0,:])])
end=time.time_ns()
print((end-start)/1E9)
start=time.time_ns()
